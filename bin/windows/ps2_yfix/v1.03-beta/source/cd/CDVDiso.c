...
//static unsigned char cdVolDescriptor[2048];
static struct dir_toc_data getDirTocData;
static struct cdVolDesc CDVolDesc;

static unsigned int last_lba;
unsigned int file_toc_offset;
unsigned int file_toc_lba;
...
void lba2msf(unsigned int lba, cdvdLoc *time){
	lba+=CD_MSF_OFFSET;
	time->frame = (unsigned char)(lba % CD_FRAMES);
	lba = lba / CD_FRAMES;
	time->minute= (unsigned char)(lba / CD_SECS);
	time->second= (unsigned char)(lba % CD_SECS);
}
...
int CdRead(unsigned int lsn, unsigned int sectors, void *buf, CdRMode *mode){
	...
	for (i=0; i<sectors; i++){
		...
		switch (mode->datapattern){
			case CdSecS2048:
				memcpy((void*)((int)buf+2048*i), buff+12+12, 2048);break;//only data
			case CdSecS2328:
				memcpy((void*)((int)buf+2328*i), buff+12+12, 2328);break;//without sync & head & sub
			case CdSecS2340:
				memcpy((void*)((int)buf+2340*i), buff+12,	 2340);break;//without sync
			default:
				return 0;
		}
	}
...
int CdWrite(unsigned int lsn, unsigned int sectors, void *buf, CdRMode *mode){
	...
	for (i=0; i<sectors; i++){
		...
		switch (mode->datapattern){
			case CdSecS2048:
				memcpy(buff+12+12, (void*)((int)buf+2048*i), 2048);break;//only data
			case CdSecS2328:
				memcpy(buff+12+12, (void*)((int)buf+2328*i), 2328);break;//without sync & head & sub
			case CdSecS2340:
				memcpy(buff+12, (void*)((int)buf+2340*i), 2340);break;//without sync
			default:
				return 0;
		}
		
		if (CDVDwriteTrack(lsn+i)==-1)
			return 0;
	}
	return 1;
}

// 2064 read stuff is not sure!
...
//		switch (mode->datapattern){
//			case CdSecS2064:
				memcpy((void*)((int)buf+2064*i), buff+12+12, 2064);break;//not sure
//			default:
//				return 0;
//		}
	}
...
//		CdSync(0x00);
	...
	while (num_dir_sectors > 0)
	{
		while(tocEntryPointer->length != 0)
		{
			...
			//if ((strnicmp(localTocEntry.filename, filename, strlen(localTocEntry.filename)) == 0)||
			//	((localTocEntry.filename[strlen(localTocEntry.filename)-2] == ';') && (strnicmp(localTocEntry.filename, filename, strlen(localTocEntry.filename)-2) == 0)))
			if ((strnicmp(localTocEntry.filename, filename, strlen(filename)) == 0)||
				((localTocEntry.filename[strlen(filename)-2] == ';') && (strnicmp(localTocEntry.filename, filename, strlen(filename)-2) == 0)))
			{
				...
				file_toc_offset = (char*)tocEntryPointer-toc;
				file_toc_lba = last_lba;
