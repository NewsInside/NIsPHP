...
//	int num_sectors;
...
#ifdef RPC_LOG
	RPC_LOG("[CDVDisodrv:write] write sectors 0x%08X to 0x%08X\n", ssector, esector-(esize==0));
#endif

	if (ssize){
		memcpy(lb + off_sector, buffer, ssize);
		if (CdWrite(ssector, 1, lb, &cdReadMode) != TRUE){
#ifdef RPC_LOG
			RPC_LOG("[CDVDisodrv:    ] Couldn't Write to file for some reason\n");
#endif
			return 0;
		}
	}
	if (asize)	if (CdWrite(asector, asize >> 11, buffer+ssize, &cdReadMode) != TRUE){
#ifdef RPC_LOG
		RPC_LOG("[CDVDisodrv:    ] Couldn't Write to file for some reason\n");
#endif
		return 0;
	...
	if (esize){
		memcpy(lb, buffer+ssize+asize, esize);
		memset(lb+esize, 0, 2048-esize);
		if (CdWrite(esector, 1, lb, &cdReadMode) != TRUE){
#ifdef RPC_LOG
			RPC_LOG("[CDVDisodrv:    ] Couldn't Read from file for some reason\n");
#endif
			return 0;
