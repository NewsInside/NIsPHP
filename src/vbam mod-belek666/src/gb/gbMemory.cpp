...
mapperMBC3 gbDataMBC3 = {
  ...
  (int)-1 // last time (time_t) => int
};

void memoryUpdateMBC3Clock()
{
  int now = time(NULL); // time_t => int
  int diff = now - gbDataMBC3.mapperLastTime; // time_t => int
  if(diff > 0) {
...
void memoryUpdateTAMA5Clock()
{
  ...
  int now = time(NULL); // (time_t) => int
  int diff = now - gbDataTAMA5.mapperLastTime; // (time_t) => int
  if(diff > 0) {
    // update the clock according to the last update time
