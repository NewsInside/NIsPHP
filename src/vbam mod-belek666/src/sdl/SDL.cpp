...
//
// You should have received a copy of sdlRtcEnablethe GNU General Public License
// along with this program; if not, write to the Free Software Foundation,
// Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.

/*#ifdef _EE
#define SYSCONFDIR "mass0:" // line 481
#endif
...
SDL_Joystick *joystick = NULL; // for pad
...
// khq2x - working low
// kAdMame2x - decent
// kBilinearPlus - looking nice but working a bit low

Filter filter =  kStretch3x;
u8 *delta = NULL;
...
char filename[2048]; // nazwa roma ewentualnie ca³a œciezka
...
int autoFrameSkip = 1; // for now on because it's too slow
...
// It's not necessery But I left it for some info about functions
// of this emu
...
// Szuka zadanego pliku (uzywane do szukania pliku konfiguracji)
// raczej nie potrzebne
int sdlFindFile(const char *name)
{
  //char buffer[4096];
 // char path[2048];
  ...
  // dorobiæ œciezke pliku
  //int f = fioOpen(path,  O_WRONLY | O_CREAT | O_BINARY);
  return 0;
  //sprintf(path2, "%s%c%s", tok, FILE_SEP, name);
}

// czyta ustawienia z pliku konfiguracyjnego vbam.cfg
// musi przyjac wskaznik na plik
...
// KarasQ: OpenGL won't working with PS2
...
// wczytuje ustawienia
// jezeli plik nie odnaleziony to uzywa domyslnych ustawien
void sdlReadPreferences()
{
  /*
  int res = sdlFindFile("vbam.cfg");
...
// wczyta ustawienia obrazu (?) z pliku vba-over.ini
// jezeli nie znajdzie pliku ustawienia domyslne obowiazuja
static void sdlApplyPerImagePreferences()
{
  /*
  int res = sdlFindFile("vba-over.ini");
  if(!res) {
...
// Zapis pliku Battery z savem
void sdlWriteBattery()
{  
  // usunac batteryDir[0] i HomeDir
  char tempname[2048];
  strcpy(tempname, filename);
  
  emulator.emuWriteBattery(strcat(tempname, ".sav"));
  systemScreenMessage("Wrote battery");
  
  printf("Wrote battery");
}

// Wczytywanie pliku battery
void sdlReadBattery()
{
  char buffer[1048];
  bool res = false;
...
// KarasQ: OpenGL won't work with PS2 so removed this part
void sdlInitVideo() {
   int flags;
   ...
   //flags |= SDL_HWSURFACE | SDL_DOUBLEBUF; // KarasQ: this make screen half-parted O_o
...
int main(int argc, char *argv[])
{
  ...
  // ---------------------- ROM LOADING ---------------------
  char *szFile = "mass:/ROM.GBA";
  
  if ( szFile ) {
    u32 len = strlen(szFile);
    if (len > SYSMSG_BUFFER_SIZE) {
      printf("STOP %i: %s: File name too long\n", __LINE__, szFile);
      while ( true );
    }
    ...
    // Sprawdza jakiego typu jest ROM
    ...
    if (type == IMAGE_GB) {
      ...
      // KarasQ: too small memory to run GBA ROM:(
      printf("running GBA\n");
      int size = CPULoadRom(szFile);
      failed = (size == 0);
      if(!failed) {

        printf("running GBA now\n");
    ...
    if(failed) {
      systemMessage(0, "STOP %i: Failed to load file %s", __LINE__, szFile);
      while(true);
    }
  } else {
...
void systemDrawScreen()
{
   ...
   filterFunction(pix + srcPitch, srcPitch, delta, screen, destPitch, srcWidth, srcHeight);
...
/////////////////////////////////////////////////////////////////////////////////////////////
      // sdlWriteBattery(); // KarasQ autosave off or my pendrive will die...
/////////////////////////////////////////////////////////////////////////////////////////////
      systemSaveUpdateCounter = SYSTEM_SAVE_NOT_UPDATED;
    }
  }
...
u32 systemReadJoypad(int which) { return inputReadJoypad(which); }

void systemUpdateMotionSensor() { inputUpdateMotionSensor(); }
